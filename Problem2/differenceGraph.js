...
//                        console.log( recent_pops[i].values[j] );
 ...
 //           }                  
       ...
       for(i=0;i<recent_pops.length;i++){
        ...
        for(i=0;i<recent_pops.length;i++){
            ...
            for(j=0;j<current_pop.length;j++){
                if( !isNaN(current_pop[j].pop) ) {
                    ...
                    //not used in the end but this would make the future predictions bigger dots than the present (useful since it's more relevant to policymaking than what the past was)                    
                    var size = 2;
        ...
        //interpolate
        for(i=0;i<recent_pops.length;i++){ //iterates through columns
            var current_pop = recent_pops[i].values;
            var j_min = 0;
            var nan_values = 0;
        ...
        //make lines
        for(i=0;i<recent_pops.length;i++){
            var current_pop = recent_pops[i].values; //console.log(current_pop);
            for(j=1;j<current_pop.length;j++){ //iterate from 1st neighbor on
                if( !isNaN(current_pop[j].pop) && !isNaN(current_pop[j-1].pop) ) {
                    ...
                    var X2 = xScale(current_pop[j+1].year); //neighbor //changed inter to have 1 line by doing j+1 as opposed to j-1 like before
                    var Y2 = yScale(current_pop[j+1].pop);  //neighbor                   
                    visFrame.attr("class", "lines")
                            .append("path")
                            .attr("d","M "+X1+" "+Y1+" L "+X2+" "+Y2+"")
                            .attr("stroke",size_scale_color(i));
                }
            }
        }        
    };